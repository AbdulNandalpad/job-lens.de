...
// CV Builder stores its data as JSON. Convert to plain text so Claude can read it.
function flattenCvJson(raw: string): string {
  if (!raw) return ''
  try {
    const d = JSON.parse(raw)
    if (typeof d !== 'object' || !d.name) return raw // already plain text
    const lines: string[] = []
        ...
        for (const part of parts) {
          ...
          try {
            ...
            switch (ev.type) {
              ...
              case 'screenshot':
                setLiveShot(ev.b64)
                addLog({ type: 'screenshot', message: ev.message, b64: ev.b64 })
                break
              case 'filling':
              ...
              case 'filled':
                ...
                break
              case 'submitting':
                addLog({ type: 'submitting', message: 'Clicking submit button...' })
                break
              case 'done':
