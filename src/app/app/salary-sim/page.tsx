...
const OUTCOME_CONFIG_DE = {
  win:     { label: 'Starker Erfolg',  color: '#1D9E75', bg: 'rgba(29,158,117,0.12)' },
  partial: { label: 'Teilerfolg',      color: '#D97706', bg: 'rgba(217,119,6,0.12)'  },
  loss:    { label: 'Standpunkt gehalten', color: '#DC2626', bg: 'rgba(220,38,38,0.12)'  },
}
...
export default function SalarySimPage() {
  ...
  const OUTCOME_CONFIG = lang === 'DE' ? OUTCOME_CONFIG_DE : OUTCOME_CONFIG_EN

  useEffect(() => { bottomRef.current?.scrollIntoView({ behavior: 'smooth' }) }, [msgs])
  useEffect(() => { if (phase === 'chat') setTimeout(() => inputRef.current?.focus(), 100) }, [phase])

  async function startNegotiation() {
    if (!role.trim()) { setError(lang === 'DE' ? 'Bitte gib die Stelle ein' : 'Please enter the role'); return }
    const offerNum  = parseFloat(offer.replace(/[^0-9.]/g, ''))
    const targetNum = parseFloat(target.replace(/[^0-9.]/g, ''))
    if (!offerNum || offerNum <= 0) { setError(lang === 'DE' ? 'Bitte gib einen gültigen Angebotsbetrag ein' : 'Please enter a valid offer amount'); return }
    if (!targetNum || targetNum <= offerNum) { setError(lang === 'DE' ? 'Das Zielgehalt muss höher als das aktuelle Angebot sein' : 'Target must be higher than the current offer'); return }
    setError('')
  ...
  async function send() {
    ...
    try {
      ...
      setMsgs(prev => { const cp = [...prev]; cp[idx] = { role: 'assistant', content: lang === 'DE' ? 'Verbindungsfehler. Bitte versuche es erneut.' : 'Connection error. Please try again.' }; return cp })
    }
    setLoading(false)
    ...
    <div style={{ minHeight: '100vh', background: c.bg, fontFamily: f.body }}>
      ...
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '24px 16px' }}>
        ...
        <div style={{ paddingLeft: 14, borderLeft: `3px solid ${c.accent}`, marginBottom: 28 }}>
          <div style={{ fontSize: 22, fontWeight: 700, color: c.primary, fontFamily: f.heading }}>{lang === 'DE' ? 'Gehaltssimulator' : 'Salary Negotiation Simulator'}</div>
          <div style={{ fontSize: 13, color: c.textMuted, marginTop: 3 }}>
            {lang === 'DE' ? 'Übe Gehaltsverhandlungen mit einem KI-Personalchef · Echter Widerstand · Sofortiges Feedback' : 'Practice negotiating with an AI hiring manager · Real pushback · Instant debrief'}
        ...
        {phase === 'setup' && (
          <div style={{ background: '#fff', border: `1px solid ${c.border}`, borderRadius: 16, padding: 28 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: c.primary, marginBottom: 20, fontFamily: f.heading }}>
              {lang === 'DE' ? 'Verhandlungsszenario einrichten' : 'Set up your negotiation scenario'}
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
              <div>
                <div style={{ fontSize: 12, fontWeight: 600, color: c.textMuted, marginBottom: 6 }}>{lang === 'DE' ? 'Stelle *' : 'Role *'}</div>
                <input value={role} onChange={e => setRole(e.target.value)}
                  placeholder={lang === 'DE' ? 'z.B. Senior Software Engineer' : 'e.g. Senior Software Engineer'}
              ...
              <div>
                <div style={{ fontSize: 12, fontWeight: 600, color: c.textMuted, marginBottom: 6 }}>{lang === 'DE' ? 'Unternehmen (optional)' : 'Company (optional)'}</div>
                <input value={company} onChange={e => setCompany(e.target.value)}
                  placeholder={lang === 'DE' ? 'z.B. Zalando' : 'e.g. Zalando'}
                  style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: `1px solid ${c.borderLight}`, fontSize: 13, fontFamily: f.body, color: c.text, outline: 'none', boxSizing: 'border-box' as const }} />
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 24 }}>
              <div>
                <div style={{ fontSize: 12, fontWeight: 600, color: c.textMuted, marginBottom: 6 }}>{lang === 'DE' ? 'Ihr Angebot (€ / Jahr) *' : 'Their offer (€ / year) *'}</div>
                <input type="number" value={offer} onChange={e => setOffer(e.target.value)}
                  placeholder={lang === 'DE' ? 'z.B. 75000' : 'e.g. 75000'}
                  style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: `1px solid ${c.borderLight}`, fontSize: 13, fontFamily: f.body, color: c.text, outline: 'none', boxSizing: 'border-box' as const }} />
              </div>
              <div>
                <div style={{ fontSize: 12, fontWeight: 600, color: c.textMuted, marginBottom: 6 }}>{lang === 'DE' ? 'Dein Zielgehalt (€ / Jahr) *' : 'Your target (€ / year) *'}</div>
                <input type="number" value={target} onChange={e => setTarget(e.target.value)}
                  placeholder={lang === 'DE' ? 'z.B. 90000' : 'e.g. 90000'}
                  style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: `1px solid ${c.borderLight}`, fontSize: 13, fontFamily: f.body, color: c.text, outline: 'none', boxSizing: 'border-box' as const }} />
              </div>
            </div>
            ...
            {offerNum > 0 && targetNum > offerNum && (
              <div style={{ marginBottom: 20, padding: '12px 16px', background: c.primaryLight, borderRadius: 10, fontSize: 13, color: c.navy }}>
                {lang === 'DE' ? 'Differenz: ' : 'Gap to close: '}<strong>{fmt(targetNum - offerNum, currency)}</strong> ({Math.round((targetNum - offerNum) / offerNum * 100)}% {lang === 'DE' ? 'Steigerung' : 'increase'})
              </div>
            )}
            ...
            <button onClick={startNegotiation}
              ...
              {lang === 'DE' ? 'Verhandlung starten →' : 'Start Negotiation →'}
            </button>

            <div style={{ marginTop: 16, fontSize: 12, color: c.textMuted, lineHeight: 1.6 }}>
              <SvgIcon name="bulb" size={13} color={c.textMuted} style={{ display: 'inline', marginRight: 4, verticalAlign: 'middle' }} /> <strong>{lang === 'DE' ? 'Tipp:' : 'Tip:'}</strong> {lang === 'DE' ? 'Der KI-Personalchef hat ein verstecktes Budgetlimit — er kann über das Erstangebot hinausgehen, aber du musst darauf drängen. Versuche hoch anzusetzen, Marktdaten zu nennen oder konkurrierende Angebote zu erwähnen.' : 'The AI hiring manager has a hidden budget ceiling — they can go higher than the initial offer, but you have to push for it. Try anchoring high, citing market data, or mentioning competing offers.'}
            </div>
          </div>
        )}
            ...
            <div style={{ padding: '14px 20px', borderBottom: `1px solid ${c.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: c.primary }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div>
                  ...
                  <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: 11 }}>{role} · {lang === 'DE' ? 'Angebot' : 'Offer'}: {fmt(offerNum, currency)}/yr · {lang === 'DE' ? 'Dein Ziel' : 'Your target'}: {fmt(targetNum, currency)}/yr</div>
                </div>
              </div>
              <button onClick={getDebrief} disabled={debriefLoading || msgs.length < 3}
                ...
                {debriefLoading ? (lang === 'DE' ? 'Wird analysiert…' : 'Analysing…') : (lang === 'DE' ? 'Beenden & Auswertung' : 'End & Debrief')}
              </button>
            </div>
            ...
            <div style={{ padding: '12px 16px 16px', borderTop: `1px solid ${c.border}` }}>
              <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end', background: c.bgSubtle, border: `1px solid ${c.borderLight}`, borderRadius: 12, padding: '8px 10px' }}>
                <textarea ref={inputRef} className="ss-input"
                  ...
                  placeholder={lang === 'DE' ? 'Mach dein Gegenangebot oder antworte Alex…' : 'Make your counter-offer or respond to Alex…'}
              ...
              <div style={{ marginTop: 6, textAlign: 'center', fontSize: 11, color: c.textFaint }}>
                {lang === 'DE'
                  ? <>Enter zum Senden · Tipp: &quot;Ich habe ein Konkurrenzangebot über {fmt(Math.round(targetNum * 0.95), currency)}…&quot; · &quot;Laut Marktdaten für Berlin…&quot;</>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ background: '#fff', border: `1px solid ${c.border}`, borderRadius: 16, padding: 28, textAlign: 'center' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'center', gap: 32, flexWrap: 'wrap' }}>
                  {[
                    { label: lang === 'DE' ? 'Erstangebot' : 'Initial offer', value: fmt(offerNum, currency), color: c.textMuted },
                    { label: lang === 'DE' ? 'Geschätztes Ergebnis' : 'Estimated result', value: fmt(debrief.final_amount, currency), color: oc.color },
                    { label: lang === 'DE' ? 'Gewonnen' : 'Gained', value: `${pct > 0 ? '+' : ''}${fmt(debrief.improvement, currency)} (${pct.toFixed(1)}%)`, color: oc.color },
                  ].map(item => (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                <div style={{ background: '#fff', border: `1px solid ${c.border}`, borderRadius: 14, padding: 20 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: '#1D9E75', letterSpacing: 0.6, textTransform: 'uppercase' as const, marginBottom: 12 }}>✓ {lang === 'DE' ? 'Was funktioniert hat' : 'What worked'}</div>
                ...
                <div style={{ background: '#fff', border: `1px solid ${c.border}`, borderRadius: 14, padding: 20 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: '#D97706', letterSpacing: 0.6, textTransform: 'uppercase' as const, marginBottom: 12 }}>↑ {lang === 'DE' ? 'Nächstes Mal' : 'Next time'}</div>
              ...
              {debrief.tactics_used.length > 0 && (
                <div style={{ background: '#fff', border: `1px solid ${c.border}`, borderRadius: 14, padding: 20 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: c.accent, letterSpacing: 0.6, textTransform: 'uppercase' as const, marginBottom: 12 }}>{lang === 'DE' ? 'Eingesetzte Taktiken' : 'Tactics you used'}</div>
              ...
              <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
                ...
                <button onClick={() => { setPhase('chat'); setDebrief(null) }}
                  ...
                  {lang === 'DE' ? 'Gespräch ansehen' : 'Review Transcript'}
                </button>
              </div>
            </div>
          )
