...
export default function CareerScanPage() {
    ...
    if (file.name.endsWith('.txt') || file.type === 'text/plain') {
      ...
      try {
        ...
        if (data.text) {
          ...
          alert(data.error || 'Could not read PDF. Please paste your CV text in the box below.')
          setFileName('')
        }
      } catch {
        alert('Failed to read PDF. Please paste your CV text in the box below.')
        setFileName('')
      }
      setFileLoading(false)
    }
  ...
  async function handleLinkedinFile(file: File) {
    ...
    try {
      ...
      const data = await res.json()
      if (data.text && !cvText) {
        // Use LinkedIn text as CV text if no CV uploaded yet
        setCvText(data.text)
      }
    } catch {
      // Non-fatal — just store filename
    }
  }

  async function runScan() {
    if (!cvText.trim()) {
      alert('Please upload your CV or paste your CV text first.')
      return
    }
    setPhase('loading')
    setMobOpen(false)
  ...
  const SB = (
    <>
      {/* LinkedIn PDF */}
      <div style={{ fontSize: 11, fontWeight: 600, color: '#042C53', textTransform: 'uppercase' as const, letterSpacing: 0.4 }}>LinkedIn PDF</div>
      <div
      ...
      >
        <input ref={linkedinInputRef} type="file" accept=".pdf" style={{ display: 'none' }}
          onChange={e => { const f = e.target.files?.[0]; if (f) handleLinkedinFile(f) }} />
        <div style={{ fontSize: 11, color: '#6b7c93', lineHeight: 1.5 }}>
          <strong style={{ display: 'block', fontSize: 12, color: '#1a2332', fontWeight: 500, marginBottom: 2 }}>
            Export from LinkedIn → Save to PDF
          </strong>
          {linkedinFileName
            ? <span style={{ color: '#0F6E56', fontWeight: 500 }}>✓ {linkedinFileName}</span>
            : 'Click to upload · PDF'}
        </div>
      </div>

      {/* CV Upload */}
      <div style={{ fontSize: 11, fontWeight: 600, color: '#042C53', textTransform: 'uppercase' as const, letterSpacing: 0.4 }}>CV / Resume</div>
      <div
      ...
      >
        ...
        <div style={{ fontSize: 11, color: '#6b7c93', lineHeight: 1.5 }}>
          <strong style={{ display: 'block', fontSize: 12, color: '#1a2332', fontWeight: 500, marginBottom: 2 }}>
            {fileLoading ? 'Reading PDF...' : fileName || 'Your CV — PDF, DOCX or TXT'}
          </strong>
          {fileLoading
            ? <span style={{ color: '#378ADD' }}>Extracting text...</span>
            : fileName
              ? <span style={{ color: '#0F6E56', fontWeight: 500 }}>✓ Ready — {cvText.length} chars extracted</span>
      ...
      {/* CV text status */}
      {fileName && cvText && (
        <div style={{ fontSize: 11, color: '#0F6E56', padding: '5px 8px', background: '#E1F5EE', borderRadius: 6 }}>
          ✓ CV text extracted successfully ({cvText.length} characters)
        </div>
      )}
      ...
      {/* Paste CV text */}
      <div style={{ fontSize: 11, fontWeight: 600, color: '#042C53', textTransform: 'uppercase' as const, letterSpacing: 0.4 }}>Or paste CV text</div>
      <textarea
        value={cvText}
        onChange={e => setCvText(e.target.value)}
        placeholder="Paste your CV content here if upload doesn't work..."
        rows={5}
      ...
      <button
        ...
        style={{
          ...
          background: canScan ? '#042C53' : '#dce4ef',
          color: canScan ? '#E6F1FB' : '#6b7c93',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
      ...
      {mode === 'roast' && (
        <div style={{ background: '#1a0a0a', border: '1.5px solid #E24B4A', borderRadius: 11, padding: 16, position: 'relative' }}>
          ...
          <button onClick={() => showToast('Roast card copied!')} style={{ width: '100%', marginTop: 10, padding: 9, borderRadius: 8, background: '#E24B4A', color: '#fff', fontFamily: "'Outfit',sans-serif", fontSize: 12, fontWeight: 600, border: 'none', cursor: 'pointer' }}>
          ...
          {[
            ...
            <div key={idx} style={{ display: 'flex', gap: 10, paddingBottom: idx < arr.length - 1 ? 16 : 0 }}>
              ...
              <div style={{ flex: 1, paddingTop: 3 }}>
                ...
                {s.tasks.map((t, ti) => (
                  <div key={ti} style={{
                    fontSize: 10, padding: '3px 8px', borderRadius: 5, marginBottom: 3,
          ...
          <div style={{ padding: 20 }}>
            {phase === 'upload' && (
              <div style={{ textAlign: 'center', padding: '60px 20px', color: '#6b7c93' }}>
                ...
                <div style={{ fontSize: 13, lineHeight: 1.7 }}>Add your CV in the sidebar, enter your target role,<br />then click Analyse My Profile.</div>
              </div>
            )}
            {phase === 'loading' && (
            ...
            {phase === 'error' && (
              <div style={{ textAlign: 'center', padding: '60px 20px' }}>
                <div style={{ fontSize: 13, color: '#E24B4A', marginBottom: 16 }}>Analysis failed. Please try again or paste your CV text manually.</div>
