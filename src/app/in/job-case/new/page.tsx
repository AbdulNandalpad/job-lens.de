...
const SBLabel = ({ children }: { children: React.ReactNode }) => (
  <div style={{ fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.45)', letterSpacing: 0.8, textTransform: 'uppercase' as const, marginBottom: 8 }}>{children}</div>
)
const SBHeading = ({ children }: { children: React.ReactNode }) => (
  <div style={{ fontFamily: f.heading, fontSize: 15, fontWeight: 700, color: '#fff', marginBottom: 6 }}>{children}</div>
)
const SBBody = ({ children }: { children: React.ReactNode }) => (
  <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.55)', lineHeight: 1.7, margin: '0 0 14px' }}>{children}</p>
)
...
const SBDivider = () => <div style={{ height: 1, background: 'rgba(255,255,255,0.07)', margin: '16px 0' }} />

function CreditsChip({ credits }: { credits: number | null }) {
  if (credits === null) return null
  const low = credits < JOB_CASE.creditCost
  return (
    <div style={{ padding: '10px 12px', background: low ? 'rgba(226,75,74,0.08)' : 'rgba(255,153,51,0.07)', borderRadius: 8, border: `1px solid ${low ? 'rgba(226,75,74,0.2)' : 'rgba(255,153,51,0.2)'}`, marginTop: 8 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
        <span style={{ fontFamily: f.heading, fontSize: 20, fontWeight: 700, color: low ? c.danger : accent }}>{credits}</span>
...
function SampleTest({ questions }: { questions: Question[] }) {
  ...
  return (
    <div style={{ marginTop: 14 }}>
      <button
        onClick={() => setOpen(o => !o)}
        style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(255,153,51,0.08)', border: '1px solid rgba(255,153,51,0.2)', borderRadius: 8, padding: '9px 12px', cursor: 'pointer', color: accent, fontSize: 12, fontWeight: 600, fontFamily: f.body }}
      >
      ...
      {open && (
        <div style={{ marginTop: 10, padding: '12px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 8 }}>
          <div style={{ fontSize: 10, color: accent, fontWeight: 700, letterSpacing: 0.5, textTransform: 'uppercase', marginBottom: 6 }}>{q.skill_being_tested}</div>
          <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.7)', lineHeight: 1.65, marginBottom: 10 }}>{q.question}</div>
          {!result ? (
            <>
              <textarea value={answer} onChange={e => setAnswer(e.target.value)} placeholder="Type your answer… aim for 100–200 words" rows={5}
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 8 }}>
                <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.25)' }}>{answer.split(/\s+/).filter(Boolean).length} words · no timer · no credits</span>
                <button onClick={score} disabled={loading}
                  style={{ background: btnGrad, color: '#fff', border: 'none', borderRadius: 6, padding: '6px 14px', fontSize: 11, fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', fontFamily: f.heading, opacity: loading ? 0.6 : 1, display: 'flex', alignItems: 'center', gap: 5 }}
                >
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                <div style={{ fontFamily: f.heading, fontSize: 26, fontWeight: 700, color: result.score >= 70 ? c.success : result.score >= 50 ? accent : c.warning, lineHeight: 1 }}>{result.score}</div>
                <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)' }}>/ 100</div>
              </div>
              <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.6)', lineHeight: 1.7, marginBottom: 10 }}>{result.feedback}</div>
              <button onClick={() => { setAnswer(''); setResult(null); setError('') }} style={{ fontSize: 11, color: accent, background: 'none', border: 'none', cursor: 'pointer', fontFamily: f.body, padding: 0, textDecoration: 'underline' }}>
                Try again
              </button>
            </div>
          )}
...
function SidebarContent({ step, credits, cvFound, cvSource, questions }: {
  ...
  if (step === 'review') return (
    <div>
      ...
      <SBBody>Your match % is based on {cvFound ? 'your CV from Career Scan' : 'a general profile estimate'}. It tells you where your strongest and weakest evidence will be before you commit.</SBBody>
      <SBDivider />
  ...
  if (step === 'consent') return (
    <div>
      ...
      <SBBody>Job Cases store sensitive data — your video and test answers. We need explicit, granular consent for each category before we store anything.</SBBody>
      <SBDivider />
      ...
      <SBDivider />
      <SBBody>You can delete your Job Case at any time from the My Cases screen and all data is removed immediately.</SBBody>
    </div>
  )
  ...
  if (step === 'questions') return (
    <div>
      ...
      <SBBody>Questions are generated specifically for this role based on what you claimed in your evidence. They cannot be googled — they require your actual experience.</SBBody>
      <SBDivider />
      ...
      <SBBody>If the questions look wrong, you can abort here and get a full credit refund. Once you move to video, credits are spent.</SBBody>
  ...
  if (step === 'test') return (
    <div>
      ...
      <SBBody>AI scores your answers for specificity, outcome clarity, and relevance. A 150-word specific answer beats a 300-word generic one every time.</SBBody>
      <SBDivider />
...
const SHARED_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; }
  ...
  .jc-input:focus { border-color: ${accent}; box-shadow: 0 0 0 3px rgba(255,153,51,0.10); }
  ...
  .jc-check { width: 16px; height: 16px; accent-color: ${accent}; cursor: pointer; flex-shrink: 0; }
...
function Card({ children, style }: { children: React.ReactNode; style?: React.CSSProperties }) {
  return <div style={{ background: '#fff', border: `1px solid ${c.border}`, borderRadius: 12, padding: '24px', boxShadow: sh.card, ...style }}>{children}</div>
}

function SectionLabel({ children }: { children: React.ReactNode }) {
  return <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: 0.8, textTransform: 'uppercase' as const, color: c.textMuted, margin: '0 0 6px' }}>{children}</p>
}
...
function StepProgress({ step }: { step: Step }) {
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      <div className="jc-step-active-label" style={{ display: 'none', fontSize: 12, fontWeight: 700, color: accent, marginBottom: 8 }}>
            ...
            <div key={s} style={{ display: 'flex', alignItems: 'center', flexShrink: 0 }}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                <div style={{
                  ...
                  background: done ? c.success : active ? accent : '#fff',
                  border: `1.5px solid ${done ? c.success : active ? accent : c.border}`,
                ...
                <span className="jc-step-label" style={{ fontSize: 9, color: active ? accent : done ? c.success : c.textFaint, fontWeight: active ? 700 : 400, whiteSpace: 'nowrap', letterSpacing: 0.3 }}>
                  {labels[i]}
                </span>
              </div>
              {i < ordered.length - 1 && (
...
function Spinner({ light = false }: { light?: boolean }) {
  return <div style={{ width: 14, height: 14, border: `2px solid ${light ? 'rgba(255,255,255,0.2)' : 'rgba(255,153,51,0.2)'}`, borderTopColor: light ? '#fff' : accent, borderRadius: '50%', animation: 'spin 0.8s linear infinite', display: 'inline-block', flexShrink: 0 }} />
}
...
export default function JobCaseNewPageIndia() {
  ...
  useEffect(() => {
    ...
    try { sessionStorage.setItem(SS.jcDraft, JSON.stringify({ jobText, jobUrl, jobTitle, company })) } catch {}
  }, [jobText, jobUrl, jobTitle, company])

  useEffect(() => {
    if (step === 'done') { try { sessionStorage.removeItem(SS.jcDraft) } catch {} }
  }, [step])
  ...
  async function analyse() {
    ...
    try {
      ...
      setStep('review')
    } catch { setAnalyseError('Network error — please try again.'); setStep('paste') }
  }
  ...
  async function uploadVideo(blob: Blob): Promise<string | null> {
    ...
    try {
      ...
      const urlRes = await fetch(API.jobCaseUploadVideo, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ mimeType }) })
            ...
            <div style={{ maxWidth: 620, margin: '0 auto', padding: '28px 20px 80px' }}>
              ...
              {(step === 'paste' || step === 'analysing') && (
                <div className="jc-fade">
                  ...
                  <Card>
                    {fromJobSearch && (
                      <div style={{ marginBottom: 12, padding: '10px 12px', background: 'rgba(255,153,51,0.06)', border: `1px solid rgba(255,153,51,0.2)`, borderRadius: 8, fontSize: 12, color: c.textMuted, lineHeight: 1.6 }}>
                        <strong style={{ color: accent }}>Pre-filled from job search.</strong> Job summaries are often shortened — visit the job posting for the full description and paste it below for the best AI matching.
                        {jobUrl && <> <a href={jobUrl} target="_blank" rel="noopener noreferrer" style={{ color: accent, marginLeft: 4 }}>Open job posting →</a></>}
                      </div>
                    )}
                    ...
                    <div style={{ marginTop: 14, padding: '12px 14px', background: cvFound ? 'rgba(29,158,117,0.06)' : cvRequired ? 'rgba(226,75,74,0.05)' : 'rgba(255,153,51,0.04)', border: `1px solid ${cvFound ? 'rgba(29,158,117,0.2)' : cvRequired ? 'rgba(226,75,74,0.3)' : c.border}`, borderRadius: 8 }}>
                      {cvFound ? (
                        ...
                        <>
                          ...
                          <div style={{ fontSize: 11, color: c.textMuted, marginBottom: 8 }}>Already did a Career Scan or CV Build? Your CV is auto-loaded. Otherwise upload below.</div>
                  ...
                  <Card style={{ marginBottom: 12 }}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, marginBottom: 18, flexWrap: 'wrap' }}>
                      ...
                      <div style={{ textAlign: 'right', flexShrink: 0 }}>
                        {matchScore !== null ? (
                          <>
                            <div style={{ fontFamily: f.heading, fontSize: 34, fontWeight: 700, color: matchScore >= 70 ? c.success : accent, lineHeight: 1 }}>{matchScore}%</div>
                    ...
                    <div style={{ marginBottom: 14, padding: '10px 12px', background: 'rgba(255,153,51,0.04)', border: `1px solid rgba(255,153,51,0.15)`, borderRadius: 8, fontSize: 12, color: c.textMuted, lineHeight: 1.6 }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {reqs.map(r => (
                        <div key={r.id} style={{ display: 'flex', gap: 10, padding: '10px 12px', background: c.bg, borderRadius: 8, border: `1px solid ${c.border}` }}>
                          <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 20, background: r.essential ? 'rgba(255,153,51,0.1)' : c.bg, color: r.essential ? accent : c.textFaint, fontWeight: 700, flexShrink: 0, alignSelf: 'flex-start', marginTop: 1, border: `1px solid ${r.essential ? 'rgba(255,153,51,0.25)' : c.border}` }}>
                      ...
                      return (
                        <div key={r.id} style={{ paddingBottom: i < reqs.length - 1 ? 22 : 0, marginBottom: i < reqs.length - 1 ? 22 : 0, borderBottom: i < reqs.length - 1 ? `1px solid ${c.border}` : 'none' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 4 }}>
                            <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 20, background: r.essential ? 'rgba(255,153,51,0.1)' : c.bg, color: r.essential ? accent : c.textFaint, fontWeight: 700, border: `1px solid ${r.essential ? 'rgba(255,153,51,0.25)' : c.border}` }}>
                              {r.essential ? 'Essential' : 'Preferred'}
                            </span>
              ...
              {step === 'questions' && (
                <div className="jc-fade">
                  ...
                  <Card style={{ marginBottom: 12 }}>
                    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '5px 12px', background: 'rgba(255,153,51,0.08)', border: `1px solid rgba(255,153,51,0.2)`, borderRadius: 20, fontSize: 12, color: accent, fontWeight: 600, marginBottom: 18 }}>
                    ...
                    {questions.map((q, i) => (
                      <div key={i} style={{ padding: '12px 14px', background: c.bg, borderRadius: 8, border: `1px solid ${c.border}`, marginBottom: i < 2 ? 10 : 0 }}>
                        <div style={{ fontSize: 10, color: accent, fontWeight: 700, marginBottom: 6, letterSpacing: 0.5, textTransform: 'uppercase' }}>Q{i + 1} · {q.skill_being_tested}</div>
                        <div style={{ fontSize: 13, color: c.text, lineHeight: 1.7 }}>{q.question}</div>
                      </div>
                    ))}
                    <div style={{ marginTop: 14, padding: '11px 14px', background: 'rgba(255,153,51,0.06)', border: `1px solid rgba(255,153,51,0.2)`, borderRadius: 8, fontSize: 13, color: c.textMuted }}>
                    ...
                    {!videoBlob ? (
                      ...
                      <div style={{ textAlign: 'center' }}>
                        ...
                        <div style={{ display: 'flex', gap: 8, justifyContent: 'center' }}>
                          ...
                          <button className="jc-btn" disabled={uploading} onClick={() => setStep('test')}>{uploading ? 'Uploading…' : 'Confirm & take test →'}</button>
                        </div>
                      </div>
                    )}
                  </Card>
                ...
                <div className="jc-fade">
                  ...
                  <Card>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18, gap: 12 }}>
                      ...
                      {testStarted && !submitted && (
                        <div style={{ padding: '6px 14px', background: timeLeft < 120 ? 'rgba(226,75,74,0.08)' : 'rgba(255,153,51,0.08)', border: `1px solid ${timeLeft < 120 ? 'rgba(226,75,74,0.2)' : 'rgba(255,153,51,0.2)'}`, borderRadius: 20, display: 'flex', alignItems: 'center', gap: 6 }}>
                          <span style={{ fontFamily: f.heading, fontSize: 16, fontWeight: 700, color: timeLeft < 120 ? c.danger : accent }}>{fmt(timeLeft)}</span>
                    ...
                    {submitted ? (
                      ...
                      <>
                        <div style={{ padding: '12px 14px', background: c.bg, borderRadius: 8, border: `1px solid ${c.border}`, marginBottom: 12 }}>
                          <div style={{ fontSize: 10, color: accent, fontWeight: 700, marginBottom: 6, letterSpacing: 0.5, textTransform: 'uppercase' }}>{questions[currentQ]?.skill_being_tested}</div>
                          <div style={{ fontSize: 13, color: c.text, lineHeight: 1.7 }}>{questions[currentQ]?.question}</div>
                        </div>
                        <textarea className="jc-input" placeholder="Type your answer… (150–250 words)" rows={8} value={answers[currentQ]}
              ...
              {step === 'generating' && (
                <div className="jc-fade" style={{ textAlign: 'center', padding: '80px 20px' }}>
                  <div style={{ width: 48, height: 48, border: `3px solid ${c.border}`, borderTopColor: accent, borderRadius: '50%', animation: 'spin 1s linear infinite', margin: '0 auto 20px' }} />
                ...
                <div className="jc-fade">
                  ...
                  <Card>
                    ...
                    <div style={{ display: 'flex', borderRadius: 8, overflow: 'hidden', border: `1px solid ${c.border}`, marginBottom: 18 }}>
                      ...
                      <button onClick={() => navigator.clipboard?.writeText(caseUrl || `https://job-lens.de/case/${caseSlug}`)}
                        style={{ padding: '10px 16px', background: btnGrad, border: 'none', color: '#fff', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: f.heading, flexShrink: 0 }}
                      >
                        Copy
                      </button>
                    </div>
